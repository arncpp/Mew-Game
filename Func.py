...
def next_track():
    ...
    :return: ничего не возвращает
    '''
    MyGlobals.current_track = (MyGlobals.current_track + 1) % MyGlobals.tracks_number
    pygame.mixer.music.load(MyGlobals.bg_music[MyGlobals.current_track])
    pygame.mixer.music.play()
...
def pet_died():
    ...
    :return: ничего не возвращает
    '''
    print_text(MyGlobals.cat.pet_name, 470 - len(MyGlobals.cat.pet_name) / 2 * 20, 150, font_color=(43, 0, 92),
               font_size=30)
    print_text(MyGlobals.cat.pet_name, 475 - len(MyGlobals.cat.pet_name) / 2 * 20, 150,
               font_color=(158, 91, 172), font_size=30)
    print_text("is dead-inside because of the", 217, 190, font_color=(43, 0, 92), font_size=30)
    print_text("is dead-inside because of the", 222, 190, font_color=(158, 91, 172), font_size=30)
    print_text(MyGlobals.cat.death_reason, 417, 230, font_color=(43, 0, 92), font_size=30)
    print_text(MyGlobals.cat.death_reason, 422, 230, font_color=(158, 91, 172), font_size=30)
    print_text("Close the game to restart", 260, 270, font_color=(43, 0, 92), font_size=30)
    print_text("Close the game to restart", 265, 270, font_color=(158, 91, 172), font_size=30)

def draw_pet_stats():
    print_text("Food:  " + str(int(MyGlobals.cat.pet_hunger)), 587, 50)
    print_text("Food:  " + str(int(MyGlobals.cat.pet_hunger)), 592, 50, font_color=(135, 0, 148))
    print_text("Sleep: " + str(int(MyGlobals.cat.pet_sleep)), 587, 90)
    print_text("Sleep: " + str(int(MyGlobals.cat.pet_sleep)), 592, 90, font_color=(135, 0, 148))
    print_text("Happy: " + str(int(MyGlobals.cat.pet_happiness)), 587, 130)
    print_text("Happy: " + str(int(MyGlobals.cat.pet_happiness)), 592, 130, font_color=(135, 0, 148))
